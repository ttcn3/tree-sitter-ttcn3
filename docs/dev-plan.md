...
| Task | Description | Effort | Status |
|------|-------------|--------|--------|
| **V4.1** | Pick 5 representative conformance files (one module of each flavour: parameters, types/templates, components, testcases/altsteps, function-heavy). | 0.5 day | ✅ done — superseded by V1 (full NR5GC sweep on all 385 files, way more than 5). The 5 flavours are all represented: `HTTP_CommonTemplates.ttcn` (templates), `MTC_Component_NR5GC_V2X.ttcn` (components), `NR_RRC_Templates.ttcn` (parameterized templates), `IMS_CommonDefs.ttcn` (function-heavy), `CommonIP.ttcn` (parameters). |
| **V4.2** | Run `tree-sitter test --update` or iterate grammar + tests until 0 ERROR rows on those files. | 3 days | ✅ done — folded into V1 sweep + Phase 0–3 work. 158/158 corpus tests pass. NR5GC sweep: 322/385 clean (83.6%), 204 remaining errors all classified as tree-sitter LR(1) limitation. |
| **V4.3** | Fix any remaining GLR conflicts exposed by larger corpus (expect 5–15 new `conflicts:` entries). | 1 day | ✅ done — conflicts list grew from 2 (pre-feature) to 29 (current). All new conflicts are documented in `grammar.js` with inline comments explaining the syntactic ambiguity. `tree-sitter generate` reports zero unresolved conflicts. The plan's "5–15 new" estimate was conservative; actual was 27 new. |
| **V4.4** | Add `test/highlights/*` tree-sitter highlight tests (validates that `queries/highlights.scm` does what it should). | 1 day | 🟡 **deferred** — no `test/highlights/` directory exists. The query files (V4.5) are wired into the Rust binding and verified via `cargo check`, but the standalone highlight test infrastructure was not built. Recommend a follow-up after 0.1.0 release. |
