...
> **Branch status (2026-07-15):** Quick Wins 3 of 5 done (QW1 done, QW3 **blocked — needs redesign**, QW4 done, QW5 done); Phase 0 expressions — 8 of 9 tasks done (E0.1, E0.2, E0.3, E0.4, E0.5, E0.6, E0.8, E0.9 landed; E0.7 `-infinity` closed via QW1). **Phase 1 templates — 4.5 of 8 tasks done** (T1.1, T1.2, T1.3, T1.4, T1.5 partial landed). 0 corpus tests failing (73/73 pass). **Real-world: HTTP_CommonTemplates.ttcn errors 17→4 (76% reduction).** See inline status notes per task.
...
| **T1.5** | Add `MatchingSymbol` alternatives: `?`, `*`, `(…)` template list, `(a..b)` range, `complement`, `pattern`, `subset`, `superset`, `permutation`, `decmatch`, `ifpresent`, length attribute. | 2 days | 🟡 partial (2026-07-15) — added 5 of 12 matching symbol alternatives: `?` (any_value), `*` (wildcard), `ifpresent`, `length(N)` (length_attribute), and `(expr..expr)` (range). **Real-world impact: HTTP_CommonTemplates.ttcn errors dropped from 17 to 4 (76% reduction)** — the `*` and `ifpresent` matching symbols are heavily used in 3GPP template bodies. The `*` rule has a deliberate conflict with the multiply operator (resolved by tree-sitter's static precedence analysis; the `primary` precedence is higher than `mul` so `*` as wildcard wins in primary position). The range rule has a deliberate conflict with `template_values` (resolved by GLR). Still TODO: complement (`!`), `pattern "…"`, `subset(…)`, `superset(…)`, `permutation(…)`, `decmatch(…)`, and the template list `(…)` form. 5 new tests in `templates.txt`. All 73/73 corpus tests pass. |
...
| 1 — Templates | 3.5–6.5 days remaining (T1.1, T1.2, T1.3, T1.4, T1.5-partial done 2026-07-15) | **yes** |
