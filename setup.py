...
class Build(build):
    def run(self):
        ...
        if not path.isfile(path.join("src", "parser.c")):
            import subprocess
            try:
        ...
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_ttcn3", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_ttcn3": ["*.pyi", "py.typed"],
        "tree_sitter_ttcn3.queries": ["*.scm"],
    },
    ext_package="tree_sitter_ttcn3",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_ttcn3/binding.c",
                "src/parser.c",
            ],
            define_macros=[
